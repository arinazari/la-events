...
- A JSON array of event records, each: `id, title, venue, neighborhood, date, start, lineup[],
  category, price, score, reasons[], lane, tags{type,genre,setting,vibe,region}`.
...
4. **adjust (−3..+3)** — a fine nudge *within* the tier, and your tool to **de-cluster** the lumpy
...
5. **lane (optional override)** — only when the deterministic lane is wrong. The classic case: a big
...
6. **why** — one line, specific, in a natural insider voice (why the verdict, especially when you
...
7. **confidence (low / med / high)** — `low` when a closer look could change the verdict (an unknown
