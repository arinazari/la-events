...
5. Commit catalog + digest with message "digest: YYYY-MM-DD (N events, M new)".
6. If the Gmail connector is available, email the digest body to Ari, subject
   "LA Events — {weekday} {M/D}".
7. If any source failed twice in a row, mark it flaky in sources.yaml and note it.
8. Do NOT run discover mode in this routine (separate weekly routine / manual).
